...
def process(dataset, model, max_emission_step, smoothing_factor):
...
if __name__ == "__main__":
	...
	if len(args) == 0:
		...
		if 'DATASET' in keys and 'MODEL' in keys:
			...
			trade_off = float(values[keys.index('TRADE_OFF')])
			...
			process(dataset, model,max_emission_step, trade_off)
		else:
