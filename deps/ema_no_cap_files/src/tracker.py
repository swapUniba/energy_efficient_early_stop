...
RESULT_PATH = os.path.join(BASE_PATH, config.get('RESULT_PATH')) ## IMPORTANTE ##
if platform == 'win32':
...
def process(dataset, model, tolerance_step, smoothing_factor):
	...
	run_recbole(
				...
				max_emission_step=tolerance_step,
...
if __name__ == "__main__":
	...
	if len(args) == 0:
		...
		if 'DATASET' in keys and 'MODEL' in keys:
			...
			tolerance_step = float(values[keys.index('TOLERANCE_STEP')])
			smoothing_factor = float(values[keys.index('SMOOTHING_FACTOR')])
			...
			process(dataset, model, tolerance_step, smoothing_factor)
		else:
